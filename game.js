...
class Game {
    ...
    start() {
        ...
        // Now initialize the game if it hasn't been already
        console.log('=== CALLING initGame FROM start() ===');
        this.initGame();
        
        // Mark the game as started and ready to play
        this.gameStarted = true;
        this.initializing = false; // Mark initialization as complete
        ...
        // Update player stats via FB Instant if available
        if (typeof FBInstant !== 'undefined' && FBInstant.player) {
            console.log("Updating FB Instant player stats");
            FBInstant.player.setDataAsync({
                gameMode: this.gameMode,
                difficulty: this.difficulty,
                lastPlayed: new Date().toISOString()
            }).catch(error => console.warn("Error updating player stats:", error));
        }
    ...
    saveHighScore(score) {
        ...
        // Save score to FB Instant leaderboard if available
        if (typeof FBInstant !== 'undefined' && FBInstant.leaderboard) {
            console.log("Saving score to FB Instant leaderboard:", score);
            FBInstant.leaderboard.setScoreAsync('bubble_shooter_main', score)
                .then(entry => console.log("Score saved to leaderboard:", entry))
    ...
    resizeCanvas() {
        ...
        // Get the game screen container for better sizing
        const gameScreen = document.getElementById('gameScreen');
        const gameContainer = document.querySelector('.game-container');
        
        // Calculate the ideal canvas size based on mobile device dimensions
        // Using common mobile aspect ratios (typically between 9:16 and 9:19)
        const idealAspectRatio = 9/19; // More elongated for modern phones
        
        // Use container dimensions if available, otherwise window dimensions
        ...
        // Calculate dimensions specifically for mobile view
        // Standard mobile resolution width is around 360-414px
        const mobileWidth = 390; // Width of typical mobile device (iPhone/Android average)
        const mobileHeight = mobileWidth / idealAspectRatio;
        
        // Calculate maximum available space with margins
        // For mobile, we want to use as much of the screen as possible
        const maxWidth = Math.min(containerWidth * 0.98, mobileWidth); 
        const maxHeight = Math.min(containerHeight * 0.95, mobileHeight);
        
        // Determine actual display dimensions maintaining aspect ratio
        let displayWidth, displayHeight;
        
        // Calculate display dimensions prioritizing height for mobile view
        if (containerHeight > containerWidth) {
            // Portrait orientation (typical mobile)
            displayWidth = maxWidth;
            displayHeight = Math.min(maxHeight, displayWidth / idealAspectRatio);
        } else {
            // Landscape orientation (tablet or desktop)
            displayHeight = maxHeight;
            displayWidth = Math.min(maxWidth, displayHeight * idealAspectRatio);
        }
        
        // Base game resolution: using mobile dimensions internally
        const baseWidth = 390;
        const baseHeight = 844; // iPhone 12/13 resolution as reference
        
        // Keep internal game dimensions consistent
        this.canvas.width = baseWidth;
        this.canvas.height = baseHeight;
        
        // Apply CSS sizing for responsive display
        ...
        document.documentElement.style.setProperty('--game-width', `${displayWidth}px`);
        document.documentElement.style.setProperty('--game-height', `${displayHeight}px`);
        
        // Log mobile-optimized dimensions
        console.log('Mobile-optimized canvas dimensions:', {
            baseWidth,
            baseHeight,
...
// This file should only contain the Game class and related game logic
// UI initialization is now handled in app.js
    ...
    // Function to start the game - exposed to window scope for FB Instant
    function startGameFunction() {
        console.log("Starting the game with the following settings:");
        ...
        try {
            ...
            if (gameScreen) gameScreen.style.display = 'flex'; // Use flex display instead of block
            document.body.classList.add('game-active'); // Add game-active class to body
            ...
            console.log("Starting the game - this will initialize everything");
    ...
    // Expose startGame function to window scope for FB Instant
    // Expose the game and start function globally for both FBInstant and direct usage
    window.startGame = startGameFunction;
    window.Game = Game; // Expose Game class
    
    // IMPORTANT: We're removing the click handler here as it's causing conflicts
    // The startup-fix.js will manage this event listener in a cleaner way
    console.log("Skipping direct startGameBtn event listener in game.js - handled by startup-fix.js");
    
    // NOTE: The original direct click handler is now removed to prevent duplicate initialization
    ...
    backToMenuBtn.addEventListener('click', () => {
        ...
        if (game) {
            // Stop game loops or animations if needed
    ...
    window.addEventListener('resize', () => {
        // If game instance exists, use its resizeCanvas method
        if (game) {
            game.resizeCanvas();
        }
        // Otherwise, just make sure the canvas is visible with proper dimensions
        else {
            const viewportWidth = window.innerWidth;
            const viewportHeight = window.innerHeight;
            const aspectRatio = 9/16; // Portrait orientation (width/height)
            ...
            // Apply dimensions via CSS for responsive display
            canvas.style.width = `${Math.floor(canvasWidth)}px`;
            canvas.style.height = `${Math.floor(canvasHeight)}px`;
        }
    });

// Explicitly expose the Game class to the window object to ensure it's available
console.log("Explicitly exposing Game class to window object");
window.Game = Game;
