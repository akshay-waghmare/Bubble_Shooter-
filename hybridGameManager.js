...
class HybridGameManager {
        ...
        try {
            ...
            switch(this.gameState.currentRendering) {
                ...
                case 'legacy':
                    ...
                    break;
                case 'compare':
                    await this.startComparisonMode();
                    break;
                default:
    ...
    async initializeGameRenderer() {
        ...
        switch(this.gameState.currentRendering) {
            ...
            case 'compare':
                ...
                // Clear comparison containers
                document.getElementById('gameContainerCompare').innerHTML = '';
                const legacyCanvas = document.getElementById('gameCanvasCompare');
                const ctx = legacyCanvas.getContext('2d');
                ctx.clearRect(0, 0, legacyCanvas.width, legacyCanvas.height);
                break;
        }
    }

    async startHybridGame() {
        // Use PhaserBubbleRenderer as a drop-in renderer for the original Game logic
        this.gameState.hybridRenderer = new PhaserBubbleRenderer('gameContainer', {
            width: 800,
            height: 600,
            use3D: true,
            quality: this.gameState.currentQuality
        });
        // Wait for Phaser to initialize (simple delay, or poll for ready)
        await new Promise(resolve => setTimeout(resolve, 500));
        // Use the original Game class, but with PhaserBubbleRenderer
        this.gameState.gameInstance = new Game(this.gameState.hybridRenderer, {
        ...
        console.log('Hybrid (Phaser+Three.js) game started');
    }
    ...
    async start2DGame() {
        ...
        await this.gameState.gameInstance.init();
    ...
    async startComparisonMode() {
        // Start legacy game in left pane
        const legacyCanvas = document.getElementById('gameCanvasCompare');
        this.legacyGameInstance = new Game(legacyCanvas, {
            mode: this.gameState.currentMode,
            difficulty: this.gameState.currentDifficulty,
            soundEnabled: this.gameState.soundEnabled
        });
        this.legacyGameInstance.start();
        // Start hybrid game in right pane
        this.hybridCompareRenderer = new HybridRenderer('gameContainerCompare', {
            width: 800,
            height: 600,
            quality: this.gameState.currentQuality,
            enableSound: this.gameState.soundEnabled,
        ...
        await this.hybridCompareRenderer.init();
        this.hybridCompareGame = new HybridBubbleShooterGame({
            renderer: this.hybridCompareRenderer,
            mode: this.gameState.currentMode,
            difficulty: this.gameState.currentDifficulty
        });
        await this.hybridCompareGame.init();
        // Store for cleanup
    ...
    stopGame() {
        ...
        if (this.legacyGameInstance && this.legacyGameInstance.destroy) {
            this.legacyGameInstance.destroy();
            this.legacyGameInstance = null;
        }
        if (this.hybridCompareGame && this.hybridCompareGame.destroy) {
            this.hybridCompareGame.destroy();
            this.hybridCompareGame = null;
        }
        if (this.hybridCompareRenderer && this.hybridCompareRenderer.destroy) {
            this.hybridCompareRenderer.destroy();
            this.hybridCompareRenderer = null;
        }
        // Hide legacy canvas
